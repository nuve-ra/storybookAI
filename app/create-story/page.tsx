  ...
  const generateStory = async () => {
    ...
    try {
      ...
      const storyTitle =
        ...
        story?.story_title ||
        'Untitled Story';
      const storyImagePrompt =
        story?.story_cover?.image_prompt ||
        story?.cover_image_prompt ||
        "A beautiful children's book cover";
      ...
      if (imageResp?.data?.imageUrl) {
        ...
        if (
          imageResp.data.base64Image &&
          imageResp.data.base64Image.length > 2 * 1024 * 1024
        ) {
          //console.warn(
          // 'Generated image is very large, this might cause issues',
          //);
